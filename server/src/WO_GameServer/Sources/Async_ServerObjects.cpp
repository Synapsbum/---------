...
CJobUpdateServerObjectVehicle::CJobUpdateServerObjectVehicle(GameObject* obj) : CAsyncApiJob()
{
	sprintf(desc, "CJobUpdateServerObjectVehicle[%s] %p", obj->Class->Name.c_str(), this);
...
int CJobUpdateServerObjectVehicle::Exec()
{
	CWOBackendReq req("api_SrvObjects.aspx");
	req.AddParam("skey1",      g_ServerApiKey);
	
	req.AddParam("func",       "updateveh");
	req.AddParam("ObjectID",   prm.ServerObjectID);
	req.AddParamF("px",        prm.CreatePos.x);
    req.AddParamF("py",        prm.CreatePos.y);
    req.AddParamF("pz",        prm.CreatePos.z);
	...
	if(!req.Issue())
	{
		r3dOutToLog("!!!! CJobUpdateServerObjectVehicle failed, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
