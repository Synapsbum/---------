...
void gameServerLoop()
{
  ...
  sprintf(fname, "logss\\%s_%I64x.txt",cfg_ginfo.name, cfg_sessionId);
  r3dChangeLogFile(fname);
  
  //sprintf(fname, "logss\\GS_%I64x.dmp", cfg_sessionId);
  sprintf(fname, "logss\\%s_%I64x.dmp",cfg_ginfo.name, cfg_sessionId);
  ...
  gMasterServerLogic.Connect("127.0.0.1", SBNET_SUPER_GAME_PORT, cfg_hostPort + SBNET_LISTEN_PORT_ADD); // 10.34.0.105
  ...
  //{
	 ...
	 // g_api_ip->SetString("api.zombie-hunters.com");
	 // gDomainUseSSL = false;
	 // gDomainPort = 80;
  //}
