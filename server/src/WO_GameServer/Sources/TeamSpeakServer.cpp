...
uint64 CTeamSpeakServer::CreateVirtualServer(const char* name, int port, int maxClients)
{
	...
	//AlexRedd:: BR mode
	//disable voip for gameServerId's here
	//if(gServerLogic.ginfo_.gameServerId == 24000)
	//return 0;
	...
	if ((error = ts3server_createVirtualServer(port, g_serverip->GetString(), name, keyPair, maxClients, &serverID)) != ERROR_ok)
