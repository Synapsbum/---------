...
int CJobProcessUserJoin::Exec()
{
	...
	// not save join info for this channels
...
//new add skin
		...
		CUserProfile::SaveGearSkinData(slot, xmlFile2);
		xml_string_writer xmlBuf2;
		xmlFile2.save(xmlBuf2, "", pugi::format_raw);
		std::string XmlGearSkinData = xmlBuf2.out_;
//new add skin
		...
		req.AddParam("sG",          XmlGearSkinData.c_str());//new add skin
...
#ifdef ENABLE_BATTLE_ROYALE
	if(updateCharEnableBR)//AlexRedd:: BR mode
	{
		const wiCharDataFull& slot = CharData;
		...
		CWOBackendReq req("api_SrvCharUpdateBR.aspx");
		req.AddSessionInfo(CustomerID, SessionID);
		req.AddParam("skey1",  g_ServerApiKey);
		req.AddParam("CharID", slot.LoadoutID);

		// character status		
		req.AddParam("sB",          GD_Diff);//update GD
		req.AddParam("s8",          slot.Stats.XP);//update XP

		UpdateChar_SetAttachments(req, slot);
		UpdateChar_SetBackpack(req, CharData, OldData);

		//trackable stats
		req.AddParam("ts03",        slot.Stats.VictorysBattleRoyale);

		if(!req.Issue())
		{
			r3dOutToLog("!!!! UpdateCharBRThread failed, code: %d, ans: %s\n", req.resultCode_, req.bodyStr_);
			return req.resultCode_;
		}
	}
#endif //ENABLE_BATTLE_ROYALE
...
int CJobAddTeleportData::Exec()
{
	//TeleportPos(0,0,0);
	CWOBackendReq req("api_SrvAddLogTeleport.aspx");
	req.AddParam("skey1", g_ServerApiKey);	
	
	req.AddParam("MapID",	 gServerLogic.ginfo_.mapId);
	char strGamePos[256];
		sprintf(strGamePos, "%.3f %.3f %.3f", TeleportPos.x, TeleportPos.y, TeleportPos.z);
	req.AddParam("TeleportPos", strGamePos);	

	// issue
	if(!req.Issue())
	{
		r3dOutToLog("!!!! AddTeleportDataThread failed, code: %d\n", req.resultCode_);
		return 0;
	}
...
int CJobBackpackFromInventory::Exec()
{
	...
	if(!req.Issue())
	{
		if(req.resultCode_ == 8)
			out_OpAns = PKT_S2C_InventoryOpAns_s::ANS_GIBanned;
		else			
			out_OpAns = PKT_S2C_InventoryOpAns_s::ANS_Fail;
...
int CJobGetLootboxData::Exec()
{
	...
	CkHttpRequest req;
	req.UsePost();
	req.put_Path("/zh/api/php/api_GetLootBoxConfig.php");
	req.AddParam("serverkey", "KfZnUUwZFJqrPJ1b4DaNZ3CkGnvz6MzPYAvP86wg");//Fg5jaBgj3uy3ja // AC44RMyzowO7ADHgQA7SKSy8I
#endif
...
#if DEV	
	CkHttpResponse* resp = http.SynchronousRequest(api_ip, 443, true, req);
#endif
...
void CJobGetLootboxData::OnSuccess()
{
	...
	//else 
	if(gServerLogic.ginfo_.channel == 4) srvLootType = 2; // premium
...
//new add skin
	...
	CUserProfile::SaveGearSkinData(*plr->loadout_, xmlFileGear);

	xml_string_writer xmlBufGear;
	xmlFileGear.save(xmlBufGear, "", pugi::format_raw);
	XmlGearSkinData = xmlBufGear.out_;
//new add skin
}
...
int CJobUpdateCharData::Exec()
{
	...
	req.AddParam("cg", XmlGearSkinData.c_str());//new add skin
	if(!req.Issue())
	{
...
int CJobRewardItem::Exec()//AlexRedd:: Item Rewards System
{
	CWOBackendReq req("api_RewardItem.aspx");
	req.AddSessionInfo(CustomerID, SessionID);
	req.AddParam("skey1",  g_ServerApiKey);
	req.AddParam("ItemID", ItemID);	
	...
	if(!req.Issue())
	{
		r3dOutToLog("!!!! RewardItemThread failed, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
...
int CJobVipItem::Exec()//AlexRedd:: Vip items (nodrop)
{
	CWOBackendReq req("api_VipItem.aspx");
	req.AddSessionInfo(CustomerID, SessionID);
	req.AddParam("skey1",  g_ServerApiKey);
	req.AddParam("ItemID", ItemID);	
	...
	if(!req.Issue())
	{
		r3dOutToLog("!!!! VipItemThread failed, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
