...
#include "ObjectsCode/obj_ServerPostBox.h"//AlexRedd:: BR mode

class GameObject;
class obj_ServerPlayer;
class obj_ServerGrenade;
class obj_ServerLockbox;
class obj_ServerAirdrop;
class ServerWeapon;
class WeaponConfig;
...
//private://AlexRedd:: BR mode 
	wiInventoryItem BRItems[72];
	int				BRItemsCount;
	int				BRBackpackId;
	void GiveDevEventLoadoutBR(DWORD peerId);

	wiInventoryItem airdropItems[72];
	int				airdropItemsCount;
	int				airdropItemID;

	wiInventoryItem airdrop2Items[72];
	int				airdrop2ItemsCount;	
	int				airdrop2ItemID;

	wiInventoryItem battleRoyaleLobbyItems[72];
	int				battleRoyaleLobbyItemsCount;
	void GiveBattleRoyaleLobbyLoadout(obj_ServerPlayer* plr);

public:
	void PreloadDevEventLoadout();
	void PreloadDevEventLoadoutSnipersMode();
	void PreloadDevEventLoadoutNoSnipersMode();
	void PreloadDevEventLoadoutBR();//AlexRedd:: BR mode 
	void PreloadAirDropLoadout();
  ...
  public:
	...
	void		GetSpawnPositionSafeZone(const r3dPoint3D& GamePos, r3dPoint3D* pos, float* dir);

	//AlexRedd:: BR mode
	/*==================================================================================================*/
#ifdef ENABLE_BATTLE_ROYALE	
	obj_ServerPlayer* winningPlr;	

	// Battle royale bool
	volatile bool		m_isGameHasStarted;
	bool		ForceStarGame;
	bool		isOccupied;	
	bool        isCountingDown;
	bool        canFinishMatch;	
	bool        isHasWinningPlayer;	
	bool		isGasSuspended;
	...
	void        GetSpawnPositionBattleRoyaleLobby(const r3dPoint3D& GamePos, r3dPoint3D* pos, float* dir);
	void        ResetPositionBattleRoyale(obj_ServerPlayer* plr, int lbNum);
	void		UpdateBattleRoyaleMatch();
	void		DestroyBarricadesBattleRoyale();
	void		ReSpawnLootBattleRoyale();
	void		ChangeGasAreaBattleRoyale();
	void		ResetGasAreaBattleRoyale();
	void		FinishMatch();	
#endif //ENABLE_BATTLE_ROYALE
	int			GetItemGDPrice(int itemID);
	void		RespawnPlayer(obj_ServerPlayer* plr, int spawnSelected);//AlexRedd:: fast respawn
	void		GiveLoadOutItems(obj_ServerPlayer* plr);//AlexRedd:: give loadout items
	void		CheckBattleZone();
	/*==================================================================================================*/	
	...
	obj_Vehicle* TryToOpenVehicleBox(obj_ServerPlayer* fromPlr, gp2pnetid_t VehicleID);
	obj_ServerAirdrop* TryToOpenAirdrop(obj_ServerPlayer* fromPlr, gp2pnetid_t AirdropID);
	...
	void		GetStartSpawnPosition(const wiCharDataFull& loadout, r3dPoint3D* pos, float* dir, DWORD peerId);//aLca :: FastloadV3
	...
	int		ProcessWorldEvent(GameObject* fromObj, DWORD eventId, DWORD peerId, const void* packetData, int packetSize);
	 ...
	 DEFINE_PACKET_FUNC(PKT_C2S_AirDropUnlock);
	...
	int       Cmd_PlayerKill(obj_ServerPlayer* plr, const char* cmd);
	int       Cmd_ServerMessage(obj_ServerPlayer* plr, const char* cmd);
	...
	int		  Cmd_StartBR(obj_ServerPlayer* plr, const char* cmd);//AlexRedd:: BR mode
	...
	void		ReadAndBroadcastMessages();//Sending automatic messages
	...
	void		InformZombiesAboutSoundItemID(const obj_ServerPlayer* plr, int ItemID);//new add
#ifdef VEHICLES_ENABLED
	void		InformZombiesAboutVehicleExplosion(const obj_Vehicle* vehicle);
#endif
	void		InformZombiesAboutExplosion(const GameObject* trg);
	...
	// async api funcs.
	//
	void		ApiPlayerUpdateWeaponSkins(obj_ServerPlayer* plr);//new add skin
	...
	struct AirDropConfigs
	{
		r3dPoint3D	m_location;
		float		m_radius;
		uint32_t	m_airDropItemID;//AlexRedd:: TODO: fix me
	};
	std::map<uint32_t, AirDropConfigs> AirDropsPos;
	...
	bool		SetAirDrop( const AirDropConfigs& AirDrop );
