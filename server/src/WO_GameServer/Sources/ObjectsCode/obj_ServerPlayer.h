...
class CJobLoadoutBackpackFromInventory;
class CJobBackpackToInventory;
class CJobLoadoutBackpackToInventory;
...
//new add skin
	...
	obj_Vehicle* GetAccessToVehicleBox(gp2pnetid_t VehicleID);
	obj_ServerAirdrop* GetAccessToAirdrop(gp2pnetid_t AirdropID);
	...
	bool		CaseAddItem(const PKT_C2S_OpenCase_s& n2, const wiInventoryItem& wi1);//AlexRedd:: case system
	void		BackpackDropItem(int slot);
	int			BackpackRemoveItemSlot(int SlotID);
	...
	bool		UseItem_CarSpawner(const r3dPoint3D& pos, float rotX, uint32_t itemID, const char* ownerUserName);//AlexRedd:: car spawner
...
private: // disable access to SetPosition directly, use TeleportPlayer
	...
	// Threads
	void Move_All_Items_Thread(const PKT_C2S_MoveAllItemsToGIReq_s& n);
	void Move_All_Items(const PKT_C2S_MoveAllItemsToGIReq_s& n, int slotID);
	void loadout_Change_Thread(const PKT_C2S_ChangeLoadoutReq_s& n);
	void loadout_Change_switch(const PKT_C2S_ChangeLoadoutReq_s& n, int function, int slotID);
...
public:
	...
	void		OnNetPacket(const PKT_C2C_PlayerReloaded_s& n);
	void		OnNetPacket(const PKT_C2S_OpenCase_s& n);//AlexRedd:: case system
	...
	void		OnNetPacket(const PKT_C2S_ChangeLoadoutReq_s& n);
	void		OnNetPacket(const PKT_C2S_MoveAllItemsToGIReq_s& n);
	...
	void		OnNetPacket(const PKT_C2S_SetRespawnData_s& n);//AlexRedd:: BR mode
	
	void		OnNetPacket(const PKT_C2S_LearnGearSkinRecipe_s& n);//new add skin
	void		OnNetPacket(const PKT_C2S_GearSkinItem_s& n);//new add skin
	...
	void		OnNetPacket(const PKT_S2C_UpdateWpnSkinSelected_s& n);//new add skin
