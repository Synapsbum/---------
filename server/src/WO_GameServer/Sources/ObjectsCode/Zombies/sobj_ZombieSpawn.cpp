...
#define CHECKRADIUS 250.f
#define SPAWN_POS_PLAYER_AROUND_CHECK 25.0f
...
BOOL obj_ZombieSpawn::OnCreate()
{
	...
	// aLca :: No1 needs this spam in the log folders
...
BOOL obj_ZombieSpawn::Update()
{
	...
	if (initialSpawning) {
		// if player is around than spawn
		if (gServerLogic.CheckForPlayersAround(GetPosition(), CHECKRADIUS))
		{
			while (zombies.size() < maxZombieCount)
			{
				if (SpawnZombie())
				{
					initialSpawning = false;
					//r3dOutToLog("Spawned zombie in init function\n");
				}
			}
		}
	...
	if (zombies.size() < maxZombieCount)
	{
		...
		{
			SpawnZombie();
			timeSinceLastSpawn = r3dGetTime() + zombieSpawnDelay;

			r3dOutToLog("Spawned zombie after LastSpawn Cooldown\n");
		}
	}
...
bool obj_ZombieSpawn::GetZombieSpawnPosition(r3dPoint3D* out_pos)
{
	...
	for (int i = 0; i<MAX_SPAWN_TRIES; i++)
	{
		...
		if (gServerLogic.CheckForPlayersAround(*out_pos, SPAWN_POS_PLAYER_AROUND_CHECK))
			continue;
