...
ZombieNavAgent* CreateZombieNavAgent(obj_Zombie* zombie)
{
	...
	a->Init(gAutodeskNavMesh.GetWorld(), zombie->GetPosition(), (zombie->IsSuperZombie() || zombie->IsSuperZombieMutant() || zombie->IsSuperZombieBerserker()) ? AutodeskNavAgent::PoiSuperZombie : AutodeskNavAgent::PoiZombie);
	a->m_pOwner = zombie;
#ifdef NEW_AI
