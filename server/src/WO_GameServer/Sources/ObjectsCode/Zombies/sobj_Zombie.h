...
public:
	...
	bool		SenseItemSound(const obj_ServerPlayer* plr, int ItemID);//new add
	...
	void		SenseExplosion(const GameObject* trg);
	...
	bool		IsSuperZombie();
	bool		IsSuperZombieMutant();
	bool		IsSuperZombieBerserker();
	void		PassiveHeal();
...
virtual	BOOL		OnNetReceive(DWORD EventID, const void* packetData, int packetSize);
	...
	bool		ApplyDamage(GameObject* fromObj, float damage, int bodyPart, STORE_CATEGORIES damageSource, uint32_t dmgItemID);
};
