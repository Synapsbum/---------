...
BOOL obj_VehicleSpawnPoint::OnCreate()
{
	...
	{
		r3dOutToLog("Vehicle LootBox: !!!! Failed to get lootbox with ID: %d. Server/Client desync??\n", m_LootBoxID);
		return FALSE;
	}
...
void obj_VehicleSpawnPoint::ReadSerializedData(pugi::xml_node& node)
{
	...
	// BR, Private and Premium servers have double the amount of vehicle spawns
	if (gServerLogic.ginfo_.channel == 1 || gServerLogic.ginfo_.channel == 3 || gServerLogic.ginfo_.channel == 4)
		maxSpawns *= 2;
}
