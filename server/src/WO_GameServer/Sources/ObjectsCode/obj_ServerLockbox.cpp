...
obj_ServerLockbox::obj_ServerLockbox()
{
	...
	maxItems2 = 50; // for now hard coded
	maxItems3 = 20; // for now hard coded
	nextInventoryID = 1;
	m_AccessCodeS[0] = 0;
	m_IsLocked   = 0;
...
BOOL obj_ServerLockbox::OnCreate()
{
	...
	{
		FileName = "Data\\ObjectsDepot\\Weapons\\Item_Lockbox_01_Big.sco";
		bsize    = r3dPoint3D(1.0900440f, 1.2519419f, 0.79267800f);
	}
	else if(m_ItemID == WeaponConfig::ITEMID_LockboxSmall)
	{
		FileName = "Data\\ObjectsDepot\\Weapons\\Item_Lockbox_01_Small.sco";
		bsize    = r3dPoint3D(1.0900440f, 1.2519419f, 0.79267800f);
	}
	else
...
bool obj_ServerLockbox::AddItemToLockbox(const wiInventoryItem& itm, int quantity)
{
	...
	if (m_ItemID == WeaponConfig::ITEMID_Lockbox)//30 slots
	{	
	...
	if (m_ItemID == WeaponConfig::ITEMID_LockboxBig)//50 slots
	{
		if (items.size() < maxItems2)
		{
			wiInventoryItem itm2 = itm;
			itm2.InventoryID = nextInventoryID++;
			itm2.quantity = quantity;
	...
	if (m_ItemID == WeaponConfig::ITEMID_LockboxSmall)//20 slots
	{
		if (items.size() < maxItems3)
		{
			wiInventoryItem itm2 = itm;
			itm2.InventoryID = nextInventoryID++;
			itm2.quantity = quantity;
