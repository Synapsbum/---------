...
{
  public:
	enum { NUM_WORKER_THREADS = 12, }; // from 4
...
class CJobAddTeleportData : public CAsyncApiJob
{
  public:
  ...
  public:
	CJobAddTeleportData() : CAsyncApiJob()
	{
		sprintf(desc, "CJobAddTeleportData"); // do not modify - it's used for reentrancy checks
		JobMaxTries = 3;	// no need to retry that job		
	}
...
class CJobUpdateCharData : public CAsyncApiJob
{
  public:
	  ...
	  std::string	XmlGearSkinData;//new add skin
...
class CJobRewardItem : public CAsyncApiJob//AlexRedd:: Item Rewards System
{
  public:
	int ItemID;	
  
  public:
	CJobRewardItem(const obj_ServerPlayer* plr) : CAsyncApiJob(plr)
	{
		sprintf(desc, "CJobRewardItem[%d] %p", CustomerID, this);
	}
...
class CJobVipItem : public CAsyncApiJob//AlexRedd:: Vip items (nodrop)
{
  public:
	int ItemID;	
  
  public:
	CJobVipItem(const obj_ServerPlayer* plr) : CAsyncApiJob(plr)
	{
		sprintf(desc, "CJobVipItem[%d] %p", CustomerID, this);
	}
