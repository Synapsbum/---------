...
#include "BattleZone.cpp"//AlexRedd:: BZ
...
void PlayGameServer()
{
  ...
  LoadLevel_Objects( 1.f );
   g_BattleZone.LoadBattleZoneGrid( r3dGameLevel::GetHomeDir() );//AlexRedd:: BZ
  ...
  if (gServerLogic.ginfo_.IsDevEvent())
  {
	...
	gServerLogic.PreloadAirDropLoadout();

#ifdef ENABLE_BATTLE_ROYALE
  //AlexRedd:: BR mode
   if (gServerLogic.ginfo_.IsGameBR()) 
	   gServerLogic.PreloadDevEventLoadoutBR(); 
  ...
  while(1) 
  {
	 // ::Sleep(10);		// limit to 45~ FPS
	  ::Sleep(25);	// aLca :: 30-34 FPS is more then fine :)
	 // ::Sleep(1);
