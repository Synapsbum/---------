...
void CMasterGameServer::Start(int port, int in_serverId)
{
  ...
  supersCooldown_  = r3dGetTime() + 3.0f; //-1; // controlled now by the number of supers connected
...
void CMasterGameServer::RequestShutdown()
{
  ...
  shuttingDown_ = true;
  shutdownLeft_ = 100.0f; // 5 min shutdown 300.0f;
...
void CMasterGameServer::Tick()
{
  ...
  // pereodically update rented servers list, every 2 min
  if(curTime > gServerConfig->nextGameServerListCheck_)
  {
    gServerConfig->nextGameServerListCheck_ = curTime + 2 * 60.0f;
...
#pragma warning( disable : 4996)
	...
	case SBPKT_G2M_SetBRGameStatus://AlexRedd:: BR mode
    {
      const SBPKT_G2M_SetBRGameStatus_s& n = *(SBPKT_G2M_SetBRGameStatus_s*)PacketData;
      r3d_assert(sizeof(n) == PacketSize);

      TGamesList::iterator it = games_.find(n.gameId);
      r3d_assert(it != games_.end());
      CServerG* game = it->second;
	  game->isBRStatus_ = n.status;

      r3dOutToLog("game %s started\n", game->GetName());      
      break;
    }
...
CServerG* CMasterGameServer::GetQuickJoinGame(int gameMap, EGBGameRegion region, int BrowseChannel, int timePlayed)
{
  ...
  for(TGamesList::iterator it = games_.begin(); it != games_.end(); ++it) 
  {
    ...
    if(gameMap < 0xFF && gameMap != gi.mapId/* && 
		BrowseChannel!=7*/)//AlexRedd:: trade mode
      continue;
...
DWORD __stdcall CMasterGameServer::ItemsDbUpdateThread(LPVOID in)
{
  ...
  try
  {
    ...
    CkHttpRequest req;
    req.UsePost();
    req.put_Path("/zh/api/php/api_getItemsDB.php");
    req.AddParam("serverkey", "KfZnUUwZFJqrPJ1b4DaNZ3CkGnvz6MzPYAvP86wg");//Fg5jaBgj3uy3ja
