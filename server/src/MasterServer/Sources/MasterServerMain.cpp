  ...
  try
  {
    ...
    while(1) 
    {
...

	  {
		  ...
		  int numLobbys = 0;
		  int numLCCU = 0;
		 /* for (CMasterGameServer::TLobbyList::const_iterator it = gMasterGameServer.lobbys_.begin();
			  it != gMasterGameServer.lobbys_.end();
			  ++it)
		  {
			  const CServerL* lobby = it->second;

			  numLCCU += lobby->curPlayers_;
			  numLobbys++;
		  }*/

		  SetConsoleTitle(Va("Master Server [Client(s) Connected: %d] - [%d Lobby(s) Online] - [%d Client(s) in Lobbys] - [%d Server(s) Online] - [%d Client(s) Ingame]", gMasterUserServer.numConnectedPeers_, numLobbys, numLCCU, numGames, numCCU));
	  }
